...
public class Generation : MonoBehaviour
{
    ...
    [Header("Минимальное расстояние от игрока до последнего фона")] [Range(0, 100)]
    ...
    [Header("Difficulty")] public float maxDifficultyByDistance;
    ...
    public CheckCollider GetCheckColliderFirst
    {
        get { return last_objects[0].GetComponent<CheckCollider>(); }
    }
    ...
    {
        GameObject newWallpaperBlockFloor =
            Instantiate(PrefabWallpaperBlockFloor, ParentPrefabWallpaperBlockFloorObjects, false);
    ...
    void CheckPlayerPosition()
    {
        ...
        var distanceBackgroundToPlayer =
            Vector3.Distance(ParentPrefabWallpaperBlockFloorObjects.GetChild(0).transform.position,
        ...
        if (distanceBackgroundToPlayer > MinimumDistanceFromPlayer)
        {
            ...
            MapTransformMesh.position = new Vector3(MapTransformMesh.position.x - 8, MapTransformMesh.position.y,
                MapTransformMesh.position.z);
    ...
    void Spawn()
    {
        ...
        if (obj == null)
        {
            Debug.LogError("Ошибка, не найден подходящий объект");
            return;
        }
    ...
    void SpawnFirst()
    {
        ...
        // print("Start First");
        // obj.StartObject = true;
        last_pref = Instantiate(
            obj.gameObject,
            Vector3.zero,
            new Quaternion(), transform);
        last_pref.GetComponent<ObjectInfo>().StartObject = true;
        ManagingMapObjects.AddAdditionalParametersCheckCollider(last_pref.GetComponent<CheckCollider>());
    ...
    {
        float angle = 45f;
        return (MAX_JAMPER_VELOCITY * MAX_JAMPER_VELOCITY * Mathf.Sin(angle) * Mathf.Sin(angle)) /
